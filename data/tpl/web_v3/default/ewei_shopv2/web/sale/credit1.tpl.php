...
<form id="setform"  <?php if(cv('sale.credit1')) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate">
    ...
    <div class="tabs-container">
		<ul class="nav nav-tabs" id="myTab">
			<li  <?php  if(empty($_GPC['tab']) || $_GPC['tab']=='basic') { ?>class="active"<?php  } ?>><a href="#tab_basic">购物送卡路里</a></li>

			<li  <?php  if($_GPC['tab']=='money') { ?>class="active"<?php  } ?>><a href="#tab_money">充值送卡路里</a></li>
		</ul>
...
<script language='javascript'>
		...
		function addConsumeItem(obj,name,enough1,enough2,give){
			...
			html+='<span class="input-group-addon">卡路里</span>';
